...
template<class T, class A> vector<T, A>::iterator vector<T, A>::insert(iterator p, const T& val)
{
	int index = p - begin();
	...
	++sz;
	iterator pp = begin() + index;	//location to put val
	for (iterator pos = end() - 1; pos != pp; --pos)
	{
		*pos = *(pos - 1);	//copy elemtns one position to the right
	}
	*(begin() + offset) = val;
	return pos;
}

template<class T, class A> vector<T, A>::iterator vector<T, A>::erase(iterator p)
{
	if (p == end()) return p;
	for (iterator pos = p + 1; pos != end(); ++pos)	//move all the elements back by one
	{
		*(pos - 1) = *pos;		//copy element one position to the left
	}
	alloc.destroy(&*pos);		//destroy surplus copy of last element, there will be two since everything was copied back
	--sz;
	return p;
}
#endif // !VECT_GUARD
